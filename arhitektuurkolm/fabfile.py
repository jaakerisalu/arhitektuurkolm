...
def defaults():
    ...
    env.service_name = ["gunicorn-arhitektuurkolm"]
    env.code_dir = '/srv/arhitektuurkolm'

@task
def test():
    defaults()
    env.hosts = ['Hostname of the staging server']
...
def live():
    ...
    env.hosts = ['Hostname of the live server']
...
def deploy(id=None, silent=False, force=False):
    ...
    if crontab_changed:
        with cd(env.code_dir):
            sudo('cp deploy/crontab.conf /etc/cron.d/arhitektuurkolm')
...
def setup_server():
    ...
    # Create database
    sudo('echo "CREATE DATABASE arhitektuurkolm; '
         '      CREATE USER arhitektuurkolm WITH password \'%s\'; '
         '      GRANT ALL PRIVILEGES ON DATABASE arhitektuurkolm to arhitektuurkolm;" '
    ...
    put(local_path=StringIO(local_settings), remote_path=env.code_dir + '/arhitektuurkolm/settings/local.py', use_sudo=True)

    # Create necessary dirs, with correct permissions
    mkdir_wwwdata('/var/log/arhitektuurkolm/')
    with cd(env.code_dir + '/arhitektuurkolm'), prefix('. ../venv/bin/activate'):
    ...
    sudo('chown -R www-data:www-data /var/log/arhitektuurkolm/')

    # Copy logrotate, nginx, crontab and gunicorn confs
    with cd(env.code_dir):
        sudo('cp deploy/logrotate.conf /etc/logrotate.d/arhitektuurkolm')
        sudo('cp deploy/%s /etc/nginx/sites-enabled/arhitektuurkolm' % env.nginx_conf)
        sudo('cp deploy/gunicorn.conf /etc/init/gunicorn-arhitektuurkolm.conf')
        sudo('cp deploy/crontab.conf /etc/cron.d/arhitektuurkolm')
...
def collectstatic():
    with cd(env.code_dir + '/arhitektuurkolm'):
